-- ============================================
-- 002 FIXED: Storage Policies + Missing Columns + New Tables
-- Fixed: FK columns use UUID to match parent tables
-- ============================================

-- 1. Storage Policies (buckets already created via JS)
CREATE POLICY IF NOT EXISTS "Authenticated users can upload payout proofs" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'payout-proofs');
CREATE POLICY IF NOT EXISTS "Public can read payout proofs" ON storage.objects FOR SELECT USING (bucket_id = 'payout-proofs');
CREATE POLICY IF NOT EXISTS "Authenticated users can upload dispute proofs" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'dispute-proofs');
CREATE POLICY IF NOT EXISTS "Public can read dispute proofs" ON storage.objects FOR SELECT USING (bucket_id = 'dispute-proofs');
...
-- 8. New tables (FK columns are UUID to match parent tables)
...
CREATE TABLE IF NOT EXISTS payout_requests (
  ...
  assigned_payouts JSONB DEFAULT '[]'::jsonb,
  status TEXT DEFAULT 'pending',
  fully_assigned BOOLEAN DEFAULT false,
