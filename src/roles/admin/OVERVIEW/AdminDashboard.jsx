...
/* ─── Color maps (same as trader panel) ─── */
const BG_MAP = {
...
/* ─── StatCard ─── */
const StatCard = ({ title, value, icon: Icon, color = 'blue', loading, subtitle, trend, className = '' }) => (
  <div
      ...
      (snap) => {
        ...
        snap.forEach(d => {
          ...
          list.push({
            id: d.id,
            type: 'warning',
            title: `Dispute: ₹${(data.amount || 0).toLocaleString()}`,
  ...
  const fetchStats = async (isRefresh = false) => {
    ...
    try {
      ...
      const [
        ...
        getDocs(query(collection(db, 'payin'), where('status', '==', 'pending'))),
