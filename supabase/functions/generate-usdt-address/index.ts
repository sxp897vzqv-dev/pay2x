 ...
 * Creates unique USDT TRC20 deposit address for a trader
 * Uses Tatum HD Wallet derivation
 */
...
serve(async (req) => {
  ...
  try {
    ...
    if (configError || !config?.master_xpub) {
      return new Response(
        JSON.stringify({ success: false, error: 'Master wallet not configured. Set up in Admin Settings first.' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
