...
-- 1. System Config table - add description column if missing
DO $$
BEGIN
...
INSERT INTO bank_health (bank_name, status) VALUES
  ...
  ('pnb', 'healthy'), ('bob', 'healthy'), ('idbi', 'healthy'),
  ('paytm', 'healthy'), ('phonepe', 'healthy'), ('gpay', 'healthy')
...
BEGIN
  UPDATE upi_pool SET
    ...
    success_rate = ROUND(((COALESCE(total_success, 0) + 1)::DECIMAL / (COALESCE(total_count, 0) + 1)::DECIMAL) * 100, 2),
    hourly_failures = 0,
    last_used_at = now(),
    updated_at = now()
  WHERE id = p_upi_id;
...
BEGIN
  UPDATE upi_pool SET
    ...
    success_rate = CASE WHEN (COALESCE(total_count, 0) + 1) > 0 
      THEN ROUND((COALESCE(total_success, 0)::DECIMAL / (COALESCE(total_count, 0) + 1)::DECIMAL) * 100, 2) 
      ELSE 0 END,
...
DROP POLICY IF EXISTS service_selection_logs ON selection_logs;
DROP POLICY IF EXISTS service_bank_health ON bank_health;
DROP POLICY IF EXISTS service_webhook_queue ON payin_webhook_queue;

CREATE POLICY service_selection_logs ON selection_logs FOR ALL USING (true);
CREATE POLICY service_bank_health ON bank_health FOR ALL USING (true);
CREATE POLICY service_webhook_queue ON payin_webhook_queue FOR ALL USING (true);
